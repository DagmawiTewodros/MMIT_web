...
const defaultItems: Item[] = [
  { id: "d1", src: beuDelivery, title: "Beu Delivery — Certification", category: "Corporate Training" },
    ...
    <SiteShell>
      ...
      {active !== null && items[active] && (
        <div className="fixed inset-0 z-[100] bg-black/85 flex items-center justify-center p-4" onClick={() => setActive(null)}>
          <button className="absolute top-4 right-4 text-white p-2 rounded-full bg-white/10 hover:bg-white/20" onClick={() => setActive(null)} aria-label="Close">
            <X className="h-6 w-6" />
          </button>
          <figure className="max-w-5xl w-full" onClick={(e) => e.stopPropagation()}>
            <img src={items[active].src} alt={items[active].title} className="w-full max-h-[80vh] object-contain rounded" />
            <figcaption className="text-center text-white mt-4">
              ...
              {items[active].description && (
                <p className="text-sm text-white/80 mt-2 max-w-2xl mx-auto">{items[active].description}</p>
              )}
            </figcaption>
          </figure>
        </div>
